//
//  ApiHandler.swift
//  WeatherAppl/Users/dcs/Documents/GitHub/WeatherApplication/WeatherApplication
//
    ...
    func searchCity(with name: String)-> [Citydetail]
    {
        ...
        if let url = URL(string: "http://api.weatherapi.com/v1/current.json?key=7dc96c4f0c7c4bdea1491232211007&q=\(name)")
        {
            ...
            if let data = try? Data(contentsOf: url)
            {
                ...
                if let jsonCity = try? decoder.decode(Citydetail.self, from: data)
                
                {
                    //print("temprature :: " + jsonCity.current.temp_c)
